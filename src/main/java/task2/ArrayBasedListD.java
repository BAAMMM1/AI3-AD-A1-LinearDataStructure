...
public class ArrayBasedListD<T> implements ILinearList<T>{
    ...
    // 4. Methoden  -> public, was wir nach außen anbieten, hierauf werden nur JUNIT-Tests geschrieben
    @Override
    public int size() {
        return size;
    }
...
//        this.tArray = new T[MAX_SIZE];              // geht bei int, String usw, aber nicht bei Generic
    ...
    public boolean equals(Object o) {
        ...
        ArrayBasedListD<?> that = (ArrayBasedListD<?>) o;

        if (Double.compare(that.maxSize, maxSize) != 0) return false;
        if (size != that.size) return false;
        // Probably incorrect - comparing Object[] arrays with Arrays.equals
        return Arrays.equals(tArray, that.tArray);
    }
